...
The contracts target Solidity `0.8.24`. See [mainnet deployment](docs/mainnet-deployment.md) to deploy a token factory, or use the [standalone private network starter](standalone/README.md) to run your own local EVM network and deploy a custom coin. A separate [standalone Stratum relay](pool/README.md) is included for self-operated proof-of-work infrastructure. Copy `.env.example` to `.env` only if you are experimenting with your own deployment environment. Never commit private keys, RPC credentials, or real wallet secrets.
